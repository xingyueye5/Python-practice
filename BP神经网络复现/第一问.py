...
for k in range(0,10):
    ...
    while(True):
        ...
        b_grad, w_grad, v_grad = [0] * 5, [0] * 4, [[0 for i in range(4)] for j in range(4)]  ##容纳每一批的梯度变化总和
        for t in range(14):
            tb_grad, tw_grad, tv_grad = [0] * 5, [0] * 4, [[0 for i in range(4)] for j in range(4)]  ##每一个x,y的临时梯度
            for j in range(4):
                s_temp=bias[j+1]
                for i in range(4):
        ...
        ##更新参数
        for i in range(5):
            bias[i]-=yita*b_grad[i]
        for i in range(4):
            w[i]-=yita*w_grad[i]
        for j in range(4):
            for i in range(4):
                v[i][j] -= yita * v_grad[i][j]
